                            ...
                            <div class="row row-deck row-cards">
                                <div class="col-md-2">
                                    <div class="form-group col-md-12">
                                        ...
                                        <input type="text" wire:model.debounce.500ms='search' class='form-control' placeholder="رقم القطعه..رقم العداد" >
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group col-md-12">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group col-md-12">
                                        <label class='form-label' for=""> نوع القطعه </label>
                                        <select wire:model='segment_type' class="form-select">
                                            <option value="all" > جميع القطع </option>
                                            @foreach ($this->segment_types as $one_segment_type)
                                            <option value='{{ $one_segment_type->id }}' > {{ $one_segment_type->name }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group col-md-12">
                                        <label class='form-label' for=""> نوع العقار </label>
                                        <select wire:model='property_type' class="form-select">
                                            <option value="all" > جميع العقارات </option>
                                            @foreach ($this->property_types as $one_property_type)
                                            <option value='{{ $one_property_type->id }}' > {{ $one_property_type->name }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group col-md-12">
                                        <label class='form-label' for=""> نوع العداد </label>
                                        <select wire:model='meter_type' class="form-select">
                                            <option value="all" > جميع العدادات </option>
                                            @foreach ($this->meter_types as $one_meter_type)
                                            <option value='{{ $one_meter_type->id }}' > {{ $one_meter_type->name }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div id="table-default" class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody class="table-tbody">
                                        ...
                                        $i = 1;
                                        @endphp
                                        @foreach ($FieldSurveys as $FieldSurvey)
                                        <tr>
                                            <td> {{ $i++ }} </td>
                                            <td> <a href="{{ route('board.workers.show' , $FieldSurvey->user_id ) }}"> {{ $FieldSurvey->user?->name }} </a> </td>
                                            <td> {{ $FieldSurvey->district?->name }} </td>
                                            <td> {{ $FieldSurvey->segment_number }} </td>
                                            <td> {{ $FieldSurvey->meter_number }} </td>
                                            <td> {{ $FieldSurvey->meters_count }} </td>
                                            <td> {{ $FieldSurvey->created_at }} <span class="text-muted"> {{ $FieldSurvey->created_at->diffForHumans() }} </span> </td>

                                            <td class='row g-2 align-items-center' >
                                                <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                                    <a href="{{ route('board.field_surveys.show' , $FieldSurvey) }}" class="btn btn-primary w-100 btn-icon" aria-label="Facebook">
                                                ...
                                                <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                                    <a class="btn btn-danger w-100 btn-icon" wire:click="$emit('confirmDeletion' , {{ $FieldSurvey->id }} )" aria-label="Facebook">
