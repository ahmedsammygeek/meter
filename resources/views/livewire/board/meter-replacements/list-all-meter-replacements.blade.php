                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row row-deck row-cards">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group col-md-12">
                                        <label class='form-label' for=""> نوع العداد الجديد </label>
                                        <select wire:model='meter_company' class="form-select">
                                            <option value="all" > جميع الانواع </option>
                                            @foreach ($this->meter_companies as $one_meter_company)
                                            <option value='{{ $one_meter_company->id }}' > {{ $one_meter_company->name }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div id="table-default" class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody class="table-tbody">
                                        ...
                                        $i = 1;
                                        @endphp
                                        @foreach ($MeterReplacements as $MeterReplacement)
                                        <tr>
                                            <td> {{ $i++ }} </td>
                                            <td> <a href="{{ route('board.workers.show' , $MeterReplacement->user_id ) }}"> {{ $MeterReplacement->user?->name }} </a> </td>
                                            <td> {{ $MeterReplacement->district?->name }} </td>
                                            <td>
                                                @switch($MeterReplacement->status)
                                                @case(1)
                                            ...
                                            <td> {{ $MeterReplacement->segment_number }} </td>
                                            <td> {{ $MeterReplacement->created_at }} <span class="text-muted"> {{ $MeterReplacement->created_at->diffForHumans() }} </span> </td>

                                            <td class='row g-2 align-items-center' >
                                                <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                                    <a href="{{ route('board.meter_replacements.show' , $MeterReplacement) }}" class="btn btn-primary w-100 btn-icon" aria-label="Facebook">
                                                ...
                                                <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                                    <a class="btn btn-danger w-100 btn-icon" wire:click="$emit('confirmDeletion' , {{ $MeterReplacement->id }} )" aria-label="Facebook">
