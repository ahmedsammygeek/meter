		...
		<div class="row g-2 align-items-center">
			...
			<div class="col-auto ms-auto d-print-none">
				<div class="btn-list">
					...
					<a href="{{ route('board.cities.index') }}" class="btn btn-primary d-none d-sm-inline-block" >
...
<div class="page-body">
	<div class="container-xl">
		<div class="row row-deck row-cards">
			<div class="col-md-12">
				<form class="card" method='POST' action='{{ route('board.cities.update' , $city ) }}' >
					@csrf
					@method('PATCH')
					...
					<div class="card-body">
						<div class="mb-3">
							...
							<div>
								<select name="area_id" class='form-control form-select' id="">
									@foreach ($areas as $area)
										<option value="{{ $area->id }}" {{ $city->area_id == $area->id ? 'selected="selected"' : '' }} > {{ $area->name }} </option>
									@endforeach
								</select>
								@error('name')
						...
						<div class="mb-3">
							<label class="form-label required"> اسم المدينه </label>
							<div>
								<input type="text" class="form-control @error('name') is-invalid @enderror " name='name' value="{{ $city->name }}" >
								@error('name')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<label class="form-check">
									<input class="form-check-input" name='active' type="checkbox" {{ $city->is_active == 1 ? 'checked' : '' }}>
